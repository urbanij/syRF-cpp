...
complex_t
calculate_vout_over_vs(complex_t y_i,
                     complex_t y_f,
                     complex_t y_o,
                     complex_t y_r,
...
double
calculate_g_s_opt(complex_t y_i,
                 complex_t y_f,
                 complex_t y_o,
                 complex_t y_r)
{
...
complex_t
calculate_y_s_opt(complex_t y_i,
                 complex_t y_f,
                 complex_t y_o,
                 complex_t y_r)
{

    double g_s_opt = calculate_g_s_opt(y_i, y_f, y_o, y_r);
    double b_s_opt = -y_i.imag() + (y_r*y_f).imag()/(2*y_o.real());

    return complex_t ( g_s_opt, b_s_opt);
}

complex_t
calculate_y_l_opt(complex_t y_i,
                 complex_t y_f,
                 complex_t y_o,
                 complex_t y_r)
{
    double g_s_opt = calculate_g_s_opt(y_i, y_f, y_o, y_r);
    double g_L_opt = (g_s_opt*y_o.real())/(y_i.real());
    double b_L_opt = -y_o.imag() + (y_r*y_f).imag()/(2*y_i.real());
